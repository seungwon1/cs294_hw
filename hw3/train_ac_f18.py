...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(str(scope)):
        for idx in range(n_layers):
            if idx == 0:
                out = tf.layers.dense(input_placeholder, size, activation=tf.tanh)
            else:
                out = tf.layers.dense(out, size, activation=tf.tanh)
                
        output_placeholder = tf.layers.dense(out, output_size, activation=output_activation)
...
class Agent(object):
    ...
    def define_placeholders(self):
        ...
        # YOUR HW2 CODE HERE
        sy_adv_n = tf.placeholder(shape=[None], name='adj', dtype=tf.float32)
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            v_next_s = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no:next_ob_no})
            target = re_n + (1-terminal_n) * self.gamma * v_next_s # (sum_of_path_lengths, ob_dim)
            for _ in range(self.num_grad_steps_per_target_update):
                self.sess.run(self.critic_update_op, feed_dict={self.sy_target_n:target, self.sy_ob_no:ob_no})
...
def train_AC(
    ...
    for itr in range(n_iter):
        ...
        advantage_n = agent.estimate_advantage(ob_no, next_ob_no, re_n, terminal_n)
        agent.update_actor(ob_no, ac_na, advantage_n)
